...
(define-public (list-asset
    ...
    (let ((listing-id (var-get listing-nonce)))
        ...
        (asserts! (> (get expiry nft-asset) stacks-block-height)
            ERR_EXPIRY_IN_PAST
        )
...
(define-public (cancel-listing
    ...
    (let (
        ...
        (asserts!
            (is-eq (get nft-asset-contract listing)
...
(define-private (assert-can-fulfil
    ...
    (begin
        ...
        (asserts! (not (is-eq (get maker listing) tx-sender))
            ERR_MAKER_TAKER_EQUAL
        )
        ...
        (asserts! (< stacks-block-height (get expiry listing))
            ERR_LISTING_EXPIRED
        )
